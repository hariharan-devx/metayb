export const getProductQueryQuery = `SELECT id, stock FROM products WHERE id=?`;
...
export const getCartQuery = `SELECT c.id,p.product,p.price,p.image,c.quantity,(p.price * c.quantity) AS total FROM cart c JOIN products p ON p.id = c.product_id WHERE c.user_id=?`;

export const updateCartQuery = `UPDATE cart SET quantity=? WHERE id=? AND user_id=?`;

export const checkCartIdQuery = `SELECT id FROM cart where id=?`;
